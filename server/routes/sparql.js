...
sparql.post('/', bodyParser.text(), async (req, res, next) => {
    try {
        ...
        let allprofiles = await profileVersionModel.find({ state: 'published' }).exec();
        allprofiles = await Promise.all(allprofiles.map(async i => profiles.profileToJSONLD(await profiles.getProfilePopulated(i.uuid))));
